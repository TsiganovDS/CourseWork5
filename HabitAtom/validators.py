...
def validate_habit_fields(habit):
    if habit.reward and habit.related_habit:
        raise ValidationError("Можно заполнить только одно из полей: вознаграждение или связанная привычка.")
...
def validate_performance(habit):
    if habit.last_performed:
        days_since_last_performance = (timezone.now().date() - habit.last_performed).days
